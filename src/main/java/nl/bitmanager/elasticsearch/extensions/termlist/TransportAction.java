...
public class TransportAction extends ShardTransportActionBase {
    ...
    protected TransportItemBase handleShardRequest(ShardRequest request) throws Exception {
        TermlistTransportItem rawRequest = (TermlistTransportItem) request.getTransportItem();

        synchronized (termlistMutex) {
            TermlistTransportItem transportItem = new TermlistTransportItem(rawRequest);
